...
func (log *Logger) Debug(ctx context.Context, msg string, args ...any) {
	// we use magic number 3
	// 0 - runtime.Callers
	// 1 - logger.write
	// 2 - logger.Debug
	// 3 - The caller of Debug, That's where we start recording
	// the return program counters of function invocations on the calling goroutine's stack
	...
	f := func(groups []string, a slog.Attr) slog.Attr {
		if a.Key == slog.SourceKey {
			if source, ok := a.Value.Any().(*slog.Source); ok {
				v := fmt.Sprintf("%s:%s:%d", filepath.Base(source.File), source.Function, source.Line)
